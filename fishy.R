...
p <- ggplot() +
  ...
  # Fish body
  geom_polygon(data = fish_data[fish_data$part == "body",], 
               aes(x = x, y = y, group = frame), 
               fill = "orange", color = "darkorange", size = 1) +
  # Tail
  geom_polygon(data = fish_data[fish_data$part == "tail",], 
               aes(x = x, y = y, group = frame), 
               fill = "orange", color = "darkorange", size = 1) +
  # Fins
  geom_polygon(data = fish_data[fish_data$part == "fin_top",], 
               aes(x = x, y = y, group = frame), 
               fill = "orange", color = "darkorange", size = 1) +
  geom_polygon(data = fish_data[fish_data$part == "fin_bottom",], 
               aes(x = x, y = y, group = frame), 
               fill = "orange", color = "darkorange", size = 1) +
  # Eye
  geom_point(data = fish_data[fish_data$part == "eye",], 
             aes(x = x, y = y, group = frame), 
             size = 3, color = "black") +
...
# Render animation
animate(p, nframes = n_frames, fps = 20, width = 600, height = 400,
